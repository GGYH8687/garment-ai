...
# =========================
# 1.1 区域 warp 强度（Stage 5.2 核心）
# =========================
UPPER_STRENGTH = 0.15
SLEEVE_STRENGTH = 0.22
SKIRT_STRENGTH = 0.10
...
# 按优先级赋值：sleeve > upper > skirt
# 先赋 skirt，再 upper，再 sleeve（后赋覆盖先赋）
...
# 边界平滑，避免区域接缝
strength_map = cv2.GaussianBlur(strength_map, (31, 31), 0)
...
cv2.imwrite(f"{OUTPUT_DIR}/warp_strength_map.png", strength_vis)
print("Saved:", f"{OUTPUT_DIR}/warp_strength_map.png")


# =========================
# 3. 收集每一行左右边界（与 gentle 版一致）
# =========================
...
#    核心改动：warp_strength → strength_map[y, x]
# =========================
...
# 9. 保存最终结果
# =========================
out_path = f"{OUTPUT_DIR}/texture_warped_regionwise_v1.png"
cv2.imwrite(out_path, result)
print("Saved:", out_path)

# 同时保存 shaded 版
out_shaded_path = f"{OUTPUT_DIR}/texture_warped_regionwise_v1_shaded.png"
cv2.imwrite(out_shaded_path, warped_shaded)
print("Saved:", out_shaded_path)
